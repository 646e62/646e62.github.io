...
[^1] As the system develops, jurisdiction may become a more significant issue. If that proves to be the case, jurisdictional functions will likely have to move to a separate class. For now, treating them as just another fact makes sense.
[^2] See note 1 above. The same logic applies to stray functions that deal with individual offences. How these are best dealt with is still very much to be determined.